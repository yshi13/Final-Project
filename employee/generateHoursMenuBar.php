...
<?php		
	...
	foreach($userJobs as $key => $val){
		...
		if ($key == 0){
			echo "\t\t\t<a href='hours.php?jobAssignmentID=$tabJobAssignmentID' class='list-group-item active text-center'>\n";
			echo "\t\t\t\t<br/>" . $val["jobName"] . "<br/><br>\n";
			echo "\t\t\t</a>\n";
					
		// other tabs are left unactivated
		} else {
			echo "\t\t\t<a href='hours.php?jobAssignmentID=$tabJobAssignmentID' class='list-group-item text-center'>\n";
			echo "\t\t\t\t<br/>" . $val["jobName"] . "<br/><br>\n";
			echo "\t\t\t</a>\n";
		};
	};
