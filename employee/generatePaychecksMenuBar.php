...
<?php		
	...
	foreach($userJobs as $key => $val){
		$tabJobAssignmentID = $val['jobAssignmentID'];
		
		// activate topmost tab
		if ($key == 0){
			echo "\t\t\t<a href='checks.php?jobAssignmentID=$tabJobAssignmentID' class='list-group-item active text-center'>\n";
			echo "\t\t\t\t<br/>" . $val["jobName"] . "<br/><br>\n";
			echo "\t\t\t</a>\n";
					
		// other tabs are left unactivated
		} else {
			echo "\t\t\t<a href='checks.php?jobAssignmentID=$tabJobAssignmentID' class='list-group-item text-center'>\n";
			echo "\t\t\t\t<br/>" . $val["jobName"] . "<br/><br>\n";
			echo "\t\t\t</a>\n";
		};
	};
